package com.github.legioth.srp4gwt.bouncycastle.util;
...
public final class BigIntegers {
    ...
    public static byte[] asUnsignedByteArray(int length, BigInteger value) {
        ...
        if (bytes[0] == 0) {
            ...
            System.arraycopy(bytes, 1, tmp, tmp.length - (bytes.length - 1),
                    bytes.length - 1);
    ...
    public static BigInteger createRandomInRange(BigInteger min,
            BigInteger max, SecureRandom random) {
        ...
        if (min.bitLength() > maxBits / 2) {
            return createRandomInRange(ZERO, max.subtract(min), random)
                    .add(min);
        }

        for (int i = 0; i < MAX_ITERATIONS; ++i) {
            BigInteger x = new BigInteger(maxBits, random);

            if (x.bitLength() > maxBits) {
                /*
                 * GWT constructor sometimes fails to produce a valid value for
                 * the bit count. Fix this by doing a quick no-op that
                 * normalizes the value.
                 */
                x = x.multiply(BigInteger.ONE);
            }
        ...
        return new BigInteger(max.subtract(min).bitLength() - 1, random)
                .add(min);
    }
}
